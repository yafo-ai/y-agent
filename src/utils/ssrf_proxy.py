...
SSRF_DEFAULT_WRITE_TIME_OUT=30
...
try:
    config_value = HTTP_REQUEST_NODE_SSL_VERIFY
    ...
    while retries <= max_retries:
        try:
            if SSRF_PROXY_ALL_URL:
                with httpx.Client(proxy=SSRF_PROXY_ALL_URL, verify=ssl_verify) as client:
                    response = client.request(method=method, url=url, **kwargs)
            elif SSRF_PROXY_HTTP_URL and SSRF_PROXY_HTTPS_URL:
                proxy_mounts = {
                    "http://": httpx.HTTPTransport(proxy=SSRF_PROXY_HTTP_URL, verify=ssl_verify),
                    "https://": httpx.HTTPTransport(proxy=SSRF_PROXY_HTTPS_URL, verify=ssl_verify),
                }
