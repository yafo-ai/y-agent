...
from src.mcp.auth.auth_provider import OAuthClientProvider
...
# 内存存储替代 Redis
oauth_state_storage: Dict[str, Tuple[float, str]] = {}  # key: (expiry_time, state_data_json)
...
def _clean_expired_states():
    ...
    with oauth_state_lock:
        for key, (expiry_time, _) in oauth_state_storage.items():
...
def _create_secure_memory_state(state_data: OAuthCallbackState) -> str:
    """Create a secure state parameter by storing state data in memory and returning a random state key."""
    # 清理过期状态
    ...
    # Store the state data in memory with expiration
    expiry_time = time.time() + OAUTH_STATE_EXPIRY_SECONDS
    memory_key = f"{OAUTH_STATE_REDIS_KEY_PREFIX}{state_key}"
    
    with oauth_state_lock:
        oauth_state_storage[memory_key] = (expiry_time, state_data.model_dump_json())

    return state_key


def _retrieve_memory_state(state_key: str) -> OAuthCallbackState:
    """Retrieve and decode OAuth state data from memory using the state key, then delete it."""
    memory_key = f"{OAUTH_STATE_REDIS_KEY_PREFIX}{state_key}"

    # Get state data from memory
    with oauth_state_lock:
        state_entry = oauth_state_storage.get(memory_key)
        
        if not state_entry:
            raise ValueError("State parameter has expired or does not exist")
            
        expiry_time, state_data_json = state_entry
        
        # Check if expired
        if time.time() > expiry_time:
            del oauth_state_storage[memory_key]
            raise ValueError("State parameter has expired")
            
        # Delete the state data from memory immediately after retrieval to prevent reuse
        del oauth_state_storage[memory_key]

    try:
        # Parse and validate the state data
        oauth_state = OAuthCallbackState.model_validate_json(state_data_json)
...
def handle_callback(state_key: str, authorization_code: str) -> OAuthCallbackState:
    ...
    full_state_data = _retrieve_memory_state(state_key)
...
def start_authorization(
    ...
    state_key = _create_secure_memory_state(state_data)
...
def auth(
    ...
    if authorization_code is not None:
        ...
        try:
            ...
            full_state_data = _retrieve_memory_state(state_param)
