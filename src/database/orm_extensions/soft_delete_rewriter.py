...
class SoftDeleteRewriter:
    ...
    def __init__(
        ...
        disable_soft_delete_option_name:
            ...
            example: User.query.execution_options(include_deleted=True)
    ...
    def rewrite_statement(self, stmt: Statement) -> Statement:
        ...
        # TODO:delete 拦截
        # 以及当启用 ORM 的 2.0 样式 select()、update() 或 delete() 构造交付给 Session.execute() 时
        # 走这个方法，需要 重写 stmt
    ...
    def rewrite_delete(self, stmt: Delete) -> Update:
        """Rewrite a Select Statement."""
        # if the user tagged this query with an execution_option to disable soft-delete filtering
        # simply return back the same stmt
        if stmt.get_execution_options().get(self.disable_soft_delete_option_name):
        ...
        raise OperationalError('不支持通过session.execute()直接删除操作，此种情况在启用软删除的时候会导致无法删除级联数据。')


        # update_stmt = table.update().where(stmt.whereclause).values([column_obj == datetime.now()])
        # update_stmt = table.update().where(stmt.whereclause).values({str(self.deleted_field_name): str(datetime.now())})

        # update_stmt3 = table.update().where(stmt.whereclause).values(self.deleted_field_name = datetime.now())
        # update_stmt = (
        #     table.update().where(table.c.id=="1")
        #     #  .filter(column_obj.is_(None))
        #      .values(deleted_at ="Patrick the Star11"))
        # # for from_obj in stmt.get_final_froms():
        # #     stmt = self.analyze_from(stmt, from_obj)
